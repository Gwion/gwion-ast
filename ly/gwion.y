...
type_decl0
  ...
  | LTMPL type_list RTMPL id_dot atsym { $$ = new_type_decl($4, $5);
      $$->types = $2; }
  | TYPEOF LPAREN id_dot RPAREN atsym { $$ = new_type_decl($3, $5); $$->xid->ref = 1; }
  ;
...
unary_exp : post_exp | unary_op unary_exp { $$ = new_exp_unary($1, $2); }
  | NEW type_decl_exp {$$ = new_exp_unary2(op_new, $2); }
  | SPORK code_stmt   { $$ = new_exp_unary3(op_spork, $2); };
  | FORK code_stmt   { $$ = new_exp_unary3(op_fork, $2); };
...
primary_exp
  ...
  | LPAREN exp RPAREN   { $$ =                      $2;                }
  | lambda_arg code_stmt { $$ = new_exp_lambda(lambda_name(arg), $1, $2); };
