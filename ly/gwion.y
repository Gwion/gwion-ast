...
%token<fval> FLOATT
%token<sval> ID "<identifier>" STRING_LIT CHAR_LIT
...
fptr_base: type_decl_array id fptr_arg { $$ = new_func_base(mpool(arg), $1, $2, $3); }
fdef_base: type_decl_empty id func_args { $$ = new_func_base(mpool(arg), $1, $2, $3); }
...
loop_stmt
  : WHILE LPAREN exp RPAREN stmt
    { $$ = new_stmt_flow(mpool(arg), ae_stmt_while, $3, $5, 0); }
  | DO stmt WHILE exp SEMICOLON
    { $$ = new_stmt_flow(mpool(arg), ae_stmt_while, $4, $2, 1); }
  ...
  | UNTIL LPAREN exp RPAREN stmt
      { $$ = new_stmt_flow(mpool(arg), ae_stmt_until, $3, $5, 0); }
  | DO stmt UNTIL exp SEMICOLON
      { $$ = new_stmt_flow(mpool(arg), ae_stmt_until, $4, $2, 1); }
...
jump_stmt
  : RETURN SEMICOLON    { $$ = new_stmt(mpool(arg), ae_stmt_return, get_pos(arg)); }
...
decl_exp2: con_exp | decl_exp3
  | AUTO atsym var_decl_list { $$= new_exp_decl(mpool(arg), new_type_decl(mpool(arg),
     new_id_list(mpool(arg), insert_symbol("auto"), get_pos(arg)), 0), $3); }
...
func_def_base
  : FUNCTION decl_template opt_flag fdef_base arg_type code_stmt
    { $$ = new_func_def(mpool(arg), $4, $6, $3 | $5, get_pos(arg));
    if($2) {
      SET_FLAG($$, template);
...
func_def
  ...
  |  OPERATOR op_op type_decl_empty LPAREN arg COMMA arg RPAREN code_stmt
    { $$ = new_func_def(mpool(arg), new_func_base(mpool(arg), $3, OP_SYM($2), $5), $9, ae_flag_op, get_pos(arg)); $5->next = $7;}
  |  OPERATOR post_op type_decl_empty LPAREN arg RPAREN code_stmt
    { $$ = new_func_def(mpool(arg), new_func_base(mpool(arg), $3, OP_SYM($2), $5), $7, ae_flag_op, get_pos(arg)); }
  |  unary_op OPERATOR type_decl_empty LPAREN arg RPAREN code_stmt
    { $$ = new_func_def(mpool(arg), new_func_base(mpool(arg), $3, OP_SYM($1), $5), $7, ae_flag_op | ae_flag_unary, get_pos(arg)); }
  | AST_DTOR code_stmt
    {
ID_List l = new_id_list(mpool(arg), insert_symbol("void"), get_pos(arg));
$$ = new_func_def(mpool(arg), new_func_base(mpool(arg), new_type_decl(mpool(arg), l, get_pos(arg)),
       insert_symbol("dtor"), NULL), $2, ae_flag_dtor, get_pos(arg)); }
  ;
